...
function StarRating({ id, initialRating, totalRating }) {
  ...
  useEffect(() => {
    document
      .querySelectorAll(`.star-rating[data-rating-id="${id}"]`)
      .forEach((el) => {
  ...
  return (
    <div className="w-fit relative -bottom-2 left-4 flex mx-2 scale-125">
      ...
      <span className="font-extralight relative -top-[0.30rem] scale-75">
        {" "}
        {userRating ? (
          <span className="text-yellow-500 font-semibold">{userRating}</span>
        ) : (
          initialRating
        )}{" "}
        ({totalRating})
