...
import { login } from "../../utils/connectToCloud";

import { AiOutlineUser } from "react-icons/ai";
import { AiFillLock } from "react-icons/ai";
import { BackToHomeBtn } from "../../components/login&Registration/subcomponents/BackToHomeBtn";
...
function LoginForm() {
  ...
  return (
    <div className="min-w-screen min-h-screen bg-gradient-to-r from-red-100 to-blue-100 grid overflow-hidden">
      <BackToHomeBtn/>
      <div className="popUp text-secondary_1 w-screen sm:w-max h-screen sm:h-auto shadow-lg pt-[20vh] sm:pt-0 m-auto backdrop-blur-lg">
        <div className="text-gray-200 bg-primary_1 text-center text-3xl font-bold px-10 py-4 ">
          LOGIN
        </div>
        <form
          action="#"
          className="py-7 sm:py-0 px-7 border-[rgba(#676F54,0.8)] border-x-2"
          method="post"
        ...
        >
          <label className="flex gap-2 pt-7">
            ...
            <input
              ...
              className="appearance-none hover:bg-gray-200  w-full p-[0.5em] sm:p-[0.2em] pl-1 rounded-lg border border-secondary_1"
              minLength="5"
          ...
          <label className="group absolute">
            <input
              ...
              id="loginBtn"
              value="LOGIN"
              className="text-gray-200 bg-primary_1 group-hover:bg-red-500 text-2xl font-semibold w-[105vw] sm:w-[19.75rem] relative top-2 -left-10 py-4 rounded-md mt-7 sm:mt-4 cursor-pointer active:scale-[101%] z-10 select-none"
            />
            ...
            <div className="relative bottom-[1.5rem] right-10 sm:right-[-16.5rem] w-0 h-0 border-solid border-t-transparent border-b-transparent border-y-[10px] border-r-red-800 group-hover:border-r-red-600 border-r-[10px] rotate-180 z-0"></div>
          </label>
        </form>
        <div className="text-primary_1 relative top-[5.5rem] px-7">
          {errorMessage}
        </div>
        <div className="text-center px-7 pt-[6.5rem] pb-4 border-x-2 border-b-2">
          A New Face?{" "}
